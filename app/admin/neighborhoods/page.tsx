...
export default function NeighborhoodsPage() {
  ...
  async function save() {
    ...
    if (error) {
      ...
      toast(editId === 'new' ? 'Bairro criado!' : 'Bairro atualizado!');
      setEditId(null);
      await load();
    }
    setSaving(false);
  ...
  async function remove(id: string) {
    ...
    if (error) {
      toast('Erro ao excluir bairro.', 'error');
    } else {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input value={form.cep_prefix} onChange={e => setForm(f => ({ ...f, cep_prefix: e.target.value.slice(0, 5) }))}
                placeholder="86010" className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none" />
            </div>
            <div>
            ...
            {rows.map(row => (
              <div key={row.id} className="bg-gray-900 p-4 rounded-lg border border-gray-800">
                ...
                <div className="grid grid-cols-2 gap-2 mt-3 text-sm">
                  <div><p className="text-gray-400">Taxa:</p><p className="font-semibold text-white">{row.delivery_fee === 0 ? 'Grátis' : `R$ ${row.delivery_fee.toFixed(2)}`}</p></div>
                  <div><p className="text-gray-400">Tempo:</p><p className="text-white">{row.estimated_minutes} min</p></div>
                </div>
              </div>
            ))}
          </div>
