...
import {
  ...
  Package, Star, Eye, EyeOff, Info, Palette,
  DollarSign, Box, Save, RotateCcw
} from 'lucide-react';
...
export default function EditProductPage() {
  ...
  async function onSubmit(data: ProductFormValues) {
    ...
    try {
      ...
      const variantsToUpsert = data.variants.map((v, i) => ({
        id: v.id,
  ...
  if (loadingData) {
    return (
      <div className="flex flex-col items-center justify-center p-24 space-y-4">
        ...
        <p className="text-gray-500 font-bold animate-pulse">Carregando produto...</p>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-grow min-w-0">
          <h1 className="text-2xl lg:text-3xl font-black text-white truncate">Editar: {productName}</h1>
          <p className="text-gray-500 text-sm font-medium">Atualize as informações e sabores do produto</p>
        </div>
        {isDirty && (
          <span className="hidden lg:flex items-center gap-1.5 text-xs font-bold text-amber-400 bg-amber-500/10 px-3 py-1.5 rounded-lg">
            <RotateCcw size={12} /> Não salvo
          </span>
        )}
      </div>
          ...
          <div className={cn('lg:col-span-7 space-y-4', activeSection !== 'flavors' && 'hidden lg:block')}>
            ...
            <div className="space-y-4">
              {fields.map((field, index) => {
                ...
                const isExisting = !!(field as any).id && originalVariants.some(ov => ov.id === (field as any).id);
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between px-4 py-3 bg-gray-800/30 border-b border-gray-800/40">
                      <div className="flex items-center gap-2">
                        ...
                        {isExisting && (
                          <span className="text-[9px] font-bold text-gray-500 bg-gray-800 px-1.5 py-0.5 rounded">
                            Existente
                          </span>
                        )}
                      </div>
        ...
        <div className="sticky bottom-0 mt-8 -mx-4 px-4 py-4 bg-gray-950/90 backdrop-blur-xl border-t border-gray-800/50 lg:-mx-0 lg:px-0 lg:bg-transparent lg:border-none lg:backdrop-blur-none">
          <div className="flex items-center justify-between gap-4">
            <div className="hidden lg:flex items-center gap-3 text-sm text-gray-500">
              ...
              {isDirty && <span className="text-amber-400 text-xs font-bold">· Alterações não salvas</span>}
            </div>
