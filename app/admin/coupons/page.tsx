...
export default function CouponsPage() {
  ...
  async function save() {
    ...
    if (error) {
      ...
      toast(editId === 'new' ? 'Cupom criado!' : 'Cupom atualizado!');
      setEditId(null);
      await load();
    }
    setSaving(false);
  ...
  async function remove(id: string) {
    ...
    if (error) {
      toast('Erro ao excluir cupom.', 'error');
    } else {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value as Coupon['type'] }))}
                className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none">
            ...
            {form.type !== 'free_delivery' && (
              <div>
                ...
                <input type="number" min="0" value={form.value} onChange={e => setForm(f => ({ ...f, value: parseFloat(e.target.value) }))}
                  className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none" />
              </div>
            )}
            <div>
              <label className="text-xs text-gray-400 uppercase tracking-wide mb-1 block">Pedido Mínimo (R$)</label>
              <input type="number" min="0" value={form.min_order_value} onChange={e => setForm(f => ({ ...f, min_order_value: parseFloat(e.target.value) }))}
                className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none" />
            </div>
            <div>
              <label className="text-xs text-gray-400 uppercase tracking-wide mb-1 block">Usos Totais (vazio = ilimitado)</label>
              <input type="number" min="1" value={form.max_uses_total ?? ''} onChange={e => setForm(f => ({ ...f, max_uses_total: e.target.value ? parseInt(e.target.value) : null }))}
                className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none" />
            </div>
            <div>
              <label className="text-xs text-gray-400 uppercase tracking-wide mb-1 block">Usos por Usuário</label>
              <input type="number" min="1" value={form.max_uses_per_user} onChange={e => setForm(f => ({ ...f, max_uses_per_user: parseInt(e.target.value) }))}
                className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none" />
            </div>
            <div>
                  ...
                  <tr key={row.id} className={`border-b border-gray-800/50 ${i % 2 === 0 ? '' : 'bg-gray-800/20'}`}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => { navigator.clipboard.writeText(row.code); toast('Código copiado!', 'info'); }} className="text-gray-500 hover:text-white"><Copy size={12} /></button>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        <button onClick={() => setConfirmId(row.id)} className="p-1.5 text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            {rows.map(row => (
              <div key={row.id} className="bg-gray-900 p-4 rounded-lg border border-gray-800">
                ...
                <div className="grid grid-cols-2 gap-2 mt-3 text-sm">
                  <div><p className="text-gray-400">Desconto:</p><p className="font-semibold text-white">{row.type === 'free_delivery' ? 'Grátis' : row.type === 'percentage' ? `${row.value}%` : `R$ ${row.value.toFixed(2)}`}</p></div>
                  <div><p className="text-gray-400">Status:</p><span className={`text-xs font-semibold px-2 py-1 rounded-full ${row.active ? 'bg-green-500/20 text-green-400' : 'bg-gray-700 text-gray-400'}`}>{row.active ? 'Ativo' : 'Inativo'}</span></div>
                </div>
              </div>
            ))}
          </div>
